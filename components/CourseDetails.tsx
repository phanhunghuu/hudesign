      ...
      <div className="space-y-8 md:space-y-16">
        {COURSES.map((course, idx) => {
          ...
          return (
            ...
            >
              <div className={`lg:w-1/3 relative flex flex-col justify-center items-center p-8 md:p-12 overflow-hidden min-h-[auto] md:min-h-[400px]`}>
                {/* Image Background */}
                <div className="absolute inset-0 z-0">
                   <img src={course.image} alt="" className="w-full h-full object-cover" />
                </div>
                {/* Gradient Overlay */}
                <div className={`absolute inset-0 z-0 ${style.overlay} backdrop-blur-[2px]`}></div>
                ...
                <div className="relative z-10 flex flex-col items-center text-center w-full space-y-4 md:space-y-8 py-4 md:py-0">
                  ...
                  <div className="space-y-0.5 md:space-y-1">
                    {course.id !== 'custom-path' && <p className="text-white/50 line-through text-[12px] md:text-lg font-bold decoration-white/30">{course.originalPrice}</p>}
                    <p className="text-3xl md:text-5xl font-black text-white drop-shadow-md">{course.discountPrice}</p>
                  </div>
                </div>
              </div>
                ...
                <div className="relative z-10">
                  ...
                  <div className="mt-8 md:mt-12 flex flex-col sm:flex-row flex-wrap items-center gap-3 md:gap-6">
                    {course.id === 'custom-path' ? (
                      <Link 
                        to="/custom-path" 
                        className={`bg-indigo-600 text-white w-full sm:w-auto px-10 md:px-12 py-4 md:py-5 rounded-xl md:rounded-2xl font-black text-sm md:text-lg hover:opacity-90 transition-all shadow-xl active:scale-95 text-center flex items-center justify-center space-x-3 group`}
                      >
                      ...
                      <>
                        <Link 
                          ...
                          className={`bg-slate-900 text-white w-full sm:w-auto px-8 md:px-10 py-4 rounded-xl md:rounded-2xl font-black text-sm md:text-base hover:opacity-90 transition-all shadow-xl active:scale-95 text-center`}
                        >
                          Đăng ký giữ chỗ
                        </Link>
                        ...
                        <Link 
                          ...
                          className={`w-full sm:w-auto border-2 ${style.textAccent} ${style.borderAccent} hover:bg-slate-50 px-8 md:px-10 py-4 rounded-xl md:rounded-2xl font-black text-sm md:text-base transition-all active:scale-95 flex items-center justify-center space-x-2`}
                        >
                          <LayoutGrid size={18} />
